...
# Grant the GitHub Actions CI role access to the EKS Kubernetes API.
# Without this the helm provider can't plan helm_release resources —
# aws eks get-token generates a valid AWS token but EKS rejects it with 401
# because the role has no access entry in the cluster.
...
module "notifications" {
  ...
  environment  = var.environment
  alert_email  = ""
  # SNS feedback role comes from IAM module; IAM does not depend on notifications → no circular dep
...
# Grant Lambda consumers KMS access to publish to encrypted SNS topics.
# This is a separate resource to avoid circular dep: IAM ← notifications ← IAM.
...
# Grant commerce-service IRSA role KMS access to publish to encrypted SNS topics.
...
module "configuration" {
  ...
  # EventBridge bus name — sourced from event_processing module output
  eventbridge_bus_name = module.event_processing.event_bus_name

  # SNS topic ARNs — enable SSM parameter writes now that topics exist
